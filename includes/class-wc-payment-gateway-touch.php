...
class WC_Gateway_Touch extends WC_Payment_Gateway {
	...
	public function process_payment( $order_id ) {
		...
		if ( $order->get_total() > 0 ) {
			$this->touch_payment_processing( $order );
		}
	}

	private function touch_payment_processing( $order ) {
		...
		$url = 'https://dev-api.gutouch.com/dist/api/touchpayapi/v1/' . $this->$partner_id . '/transaction?loginAgent=' . $this->$login_agent.'&passwordAgent=' . $this->$password_agent;
		var_dump($url);
